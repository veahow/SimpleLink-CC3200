...
/*******************************************************************************
 ...
 返回值：无
*******************************************************************************/
void TMP_Read(unsigned int *temp_int, unsigned int *temp_dec)
{
  I2C_IF_Open(I2C_MASTER_MODE_FST);
  ...
  if(!iRetVal){
      ...
      if(!(usTemp & 0x8000)){
        ...
        *temp_int = usTemp >> 7;
        *temp_dec = ((usTemp&0x7f)*25)>>5;
        
        snprintf(str, sizeof(str), "当前温度值是: %d.%02d℃\n\r", *temp_int, *temp_dec);    // 显示整数部分 & 显示两位小数
        Report(str);
      }
  } 
...
/*******************************************************************************
 ...
 返回值：无
*******************************************************************************/
void BMA_Read(unsigned char *ax, unsigned char *ay, unsigned char *az)
{
  I2C_IF_Open(I2C_MASTER_MODE_FST);
  ...
  if(!iRetVal){
      ...
      *az = ucData[5];
      ...
      Report("x: %03d  ", *ax);    // 显示x轴加速度
      Report("y: %03d  ", *ay);    // 显示y轴加速度
      Report("z: %03d\n\r", *az);    // 显示z轴加速度
   }
